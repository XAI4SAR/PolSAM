...
class Prompt(nn.Module):
    ...
    # def __init__(self, embed_dims):
        super().__init__()
        self.pt = nn.Linear(768, 192)  # vit-b
        # self.pt = nn.Linear(1024, 256)  # vit-l
    def forward(self, x: torch.Tensor):
        x = x.permute(0,3,1,2)
        x = x.flatten(2).transpose(1,2)  # [12, 1024, 768]
        x = self.pt(x)
        return x
...
class Prompt_block(nn.Module, ):
    # def __init__(self, inplanes=1024, hide_channel=8, smooth=True):
    def __init__(self, inplanes=768, hide_channel=8, smooth=True):
...
class CrossPath(nn.Module):
    ...
    def forward(self, x1, x2):  # [16, 4096, 128])
...
class ChannelEmbed(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, _C = x.shape
        x = x.permute(0, 2, 1).reshape(B, _C, H, W).contiguous()  # [2, 384, 32, 32])
        residual = self.residual(x)  # [2, 192, 32, 32]
...
class FeatureFusionModule(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x2 = x2.flatten(2).transpose(1, 2)  # [16, 4096, 128]
        x1, x2 = self.cross(x1, x2)  # [b,1024,c]
